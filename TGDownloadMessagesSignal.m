...
static inline int64_t TGIOS6DownloadChannelIdFromPeerId(int64_t peerId, int64_t accessHash)
{
    ...
    if (channelId != 0)
        return (int64_t)(uint32_t)channelId;
    ...
    if (channelMessageIdsByPeerId.count != 0) {
        ...
        [channelMessageIdsByPeerId enumerateKeysAndObjectsUsingBlock:^(__unused id key, NSArray *messages, __unused BOOL *stop) {
            if (messages.count != 0) {
                ...
                TGLog(@"IOS6TRACE channels.getMessages peer=%lld modernRaw=%d channel=%lld hash=%lld count=%d firstMid=%d", firstMessage.peerId, TGIOS6DownloadPeerIdIsModernRawChannel(firstMessage.peerId, firstMessage.accessHash) ? 1 : 0, inputChannel.channel_id, inputChannel.access_hash, (int)messageIds.count, firstMessage.messageId);
...
+ (SSignal *)mediaStickerpacks:(TGMediaAttachment *)attachment {
    TGLog(@"IOS6MEDIA stickers disabled: skip attached sticker packs for %@", NSStringFromClass([attachment class]));
    return [SSignal single:@[]];
    ...
    switch (origin.type) {
        case TGMediaOriginTypeMessage:
        {
            SSignal *accessHashSignal = [SSignal defer:^SSignal *{
                ...
                TGLog(@"IOS6MEDIA origin refresh peer=%lld mid=%d accessHash=%lld", origin.cid.longLongValue, origin.mid.intValue, accessHash);
            ...
            }] map:^id(NSArray *messages)
            {
                ...
                if (message == nil) {
                    TGLog(@"IOS6MEDIA origin refresh empty peer=%lld mid=%d", origin.cid.longLongValue, origin.mid.intValue);
                }
        ...
        {
            TGLog(@"IOS6MEDIA stickers disabled: skip sticker origin refresh pack=%@", origin.stickerPackId);
            return [SSignal single:@{}];
        }
            break;
        ...
        {
            TGLog(@"IOS6MEDIA stickers disabled: skip recent sticker origin refresh");
            return [SSignal single:@{}];
        }
            break;
