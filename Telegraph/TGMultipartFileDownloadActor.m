...
@property (nonatomic) int requestGeneration;
@property (nonatomic) int timeoutRetryCount;
...
- (void)execute:(NSDictionary *)options
{
    ...
    _identifier = [options[@"identifier"] longLongValue];
    ...
    if (_fileLocation != nil && _storeFilePaths.count != 0)
    {
        ...
        // A second connection only helps large, known-size files.  Previously
        // every thumbnail and sticker reserved two of the four media workers,
        // so two stalled items could block all media in a chat.
...
#ifdef DEBUG
    IOS6_NOOP_LOG(@"Average speed: %f kbytes/s", _averageDownloadSpeed / 1024.0f);
#endif
}
...
- (void)scheduleTimeoutForPartOffset:(int)offset generation:(int)generation token:(id)token
{
    __weak TGMultipartFileDownloadActor *weakSelf = self;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(30.0 * NSEC_PER_SEC)), [ActionStageInstance() globalStageDispatchQueue], ^
    {
        [ActionStageInstance() dispatchOnStageQueue:^
        {
            __strong TGMultipartFileDownloadActor *strongSelf = weakSelf;
            if (strongSelf == nil || strongSelf.cancelled)
                return;

            TGFilePartInfo *partInfo = strongSelf->_downloadingParts[@(offset)];
            if (partInfo == nil || partInfo.requestGeneration != generation || ![partInfo.token isEqual:token])
                return;

            // Slow links are allowed to continue while bytes are still moving.
            if (CFAbsoluteTimeGetCurrent() - partInfo.lastActivityTime < 25.0)
            {
                [strongSelf scheduleTimeoutForPartOffset:offset generation:generation token:token];
                return;
            }

            [TGTelegraphInstance cancelRequestByToken:token];
            [strongSelf->_worker1.strongWorker cancelRequestById:token];
            [strongSelf->_worker2.strongWorker cancelRequestById:token];
            partInfo.token = nil;
            partInfo.downloadedLength = 0;

            // Retry a silent request once.  A second timeout is reported to
            // the fetcher instead of keeping a worker and a 0.00 MB UI forever.
            if (partInfo.timeoutRetryCount < 1)
            {
                partInfo.timeoutRetryCount++;
                partInfo.restart = true;
                [strongSelf downloadFileParts];
            }
            else
            {
                [strongSelf filePartDownloadFailed:nil offset:offset length:partInfo.partLength];
            }
        }];
    });
}
...
- (void)downloadFileParts
{
    ...
    else
    {
        ...
        // Four small MTProto parts keep one connection saturated without
        // consuming another worker. Unknown-size legacy resources remain
        // strictly sequential so their end is detected safely.
        int maximumActiveParts = _size != 0 ? 4 : 1;
        while (activeDownloadingParts < maximumActiveParts)
        {
        ...
        for (TGFilePartRequestInfo *requestInfo in requestList)
        {
            ...
            TGFilePartInfo *scheduledPartInfo = _downloadingParts[@(requestInfo.offset)];
            if (scheduledPartInfo == nil)
                continue;

            scheduledPartInfo.requestGeneration++;
            int requestGeneration = scheduledPartInfo.requestGeneration;
            scheduledPartInfo.startTime = CFAbsoluteTimeGetCurrent();
            scheduledPartInfo.lastActivityTime = scheduledPartInfo.startTime;
            id token = request.internalId;
            scheduledPartInfo.token = token;
            ...
            [request setCompleted:^(TLupload_File *result, __unused NSTimeInterval timestamp, id error)
            {
                [ActionStageInstance() dispatchOnStageQueue:^
                {
                    ...
                    if (strongSelf != nil) {
                        TGFilePartInfo *currentPartInfo = strongSelf->_downloadingParts[@(requestInfo.offset)];
                        if (currentPartInfo == nil || currentPartInfo.requestGeneration != requestGeneration || ![currentPartInfo.token isEqual:token])
                            return;
                        currentPartInfo.token = nil;
                        ...
                        else
                        {
                            ...
                            if (strongSelf->_originInfo != nil && errorCode == 400 && [errorString hasPrefix:@"FILE_REFERENCE_"] && !strongSelf->_retrying)
                            {
                                ...
                                while (false) TGLog(@"IOS6MEDIA multipart fileRef error text=%@ offset=%d", errorString, requestInfo.offset);
                                [strongSelf updateFileReference:strongSelf->_originInfo offset:requestInfo.offset length:requestInfo.length];
                            }
                            else if (errorCode == 400 && ([errorString isEqualToString:@"PEER_ID_INVALID"] || [errorString isEqualToString:@"CHANNEL_INVALID"]) && !strongSelf->_retrying && [strongSelf->_fileLocation isKindOfClass:[TLInputFileLocation$inputPeerPhotoFileLocation class]])
                            {
                                TLInputFileLocation$inputPeerPhotoFileLocation *peerPhotoLocation = (TLInputFileLocation$inputPeerPhotoFileLocation *)strongSelf->_fileLocation;
                                if (peerPhotoLocation.peer_id > 0 && peerPhotoLocation.peer_id < INT32_MAX)
                                {
                                    ((TGFilePartInfo *)strongSelf->_downloadingParts[@(requestInfo.offset)]).restart = true;
                                    strongSelf->_retrying = true;
                                    int64_t oldPeerId = peerPhotoLocation.peer_id;
                                    peerPhotoLocation.peer_id = oldPeerId + ((int64_t)UINT32_MAX + 1);
                                    while (false) TGLog(@"IOS6MEDIA multipart peerphoto retry unsigned type=%d error=%@ legacy=%lld modern=%lld photo=%lld", peerPhotoLocation.peer_type, errorString, oldPeerId, peerPhotoLocation.peer_id, peerPhotoLocation.photo_id);
                                    [strongSelf downloadFileParts];
                                }
                                else
                                {
            ...
            [request setProgressUpdated:^(float progress, NSUInteger packetLength)
            {
                [ActionStageInstance() dispatchOnStageQueue:^
                {
                    ...
                    if (strongSelf != nil)
                    {
                        ...
                        if (currentPartInfo != nil && currentPartInfo.requestGeneration == requestGeneration && [currentPartInfo.token isEqual:token])
                        {
                            currentPartInfo.lastActivityTime = CFAbsoluteTimeGetCurrent();
            ...
            [self scheduleTimeoutForPartOffset:requestInfo.offset generation:requestGeneration token:token];
        }
    }
}
    ...
    [_fileReferenceDisposable setDisposable:[[TGDownloadMessagesSignal updatedOriginInfo:originInfo identifier:_identifier] startWithNext:^(TGMediaOriginInfo *next)
    {
        [ActionStageInstance() dispatchOnStageQueue:^{
            ...
            if (strongSelf != nil) {
                ...
                strongSelf->_retrying = true;
                NSData *updatedFileReference = nil;
                if ([strongSelf->_fileLocation isKindOfClass:[TLInputFileLocation$inputDocumentFileLocation class]]) {
                    ...
                    updatedFileReference = [next fileReferenceForDocumentId:location.n_id accessHash:location.access_hash];
                    if (updatedFileReference.length == 0)
                        updatedFileReference = [next fileReference];
                    location.file_reference = updatedFileReference;
                    ...
                    updatedFileReference = [next fileReferenceForVolumeId:location.volume_id localId:location.local_id];
                    location.file_reference = updatedFileReference;
                }
                
                if (next == nil || updatedFileReference.length == 0)
                {
                    while (false) TGLog(@"IOS6MEDIA multipart fileRef refresh empty identifier=%lld", strongSelf->_identifier);
                    [strongSelf filePartDownloadFailed:nil offset:offset length:length];
                    return;
                }
                while (false) TGLog(@"IOS6MEDIA multipart fileRef refreshed identifier=%lld", strongSelf->_identifier);
                [strongSelf downloadFileParts];
            }
        }];
