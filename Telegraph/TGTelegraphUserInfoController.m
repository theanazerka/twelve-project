...
- (id)_externalShareItemForUser:(TGUser *)user
{
    ...
    if (iosMajorVersion() >= 9)
    {
        ...
        CNLabeledValue *phoneValue = [CNLabeledValue labeledValueWithLabel:nil value:[CNPhoneNumber phoneNumberWithStringValue:_user.formattedPhoneNumber]];
...
- (TGModernGalleryController *)createAvatarGalleryControllerForPreviewMode:(bool)previewMode
{
    ...
    if (user.photoUrlSmall.length != 0 || user.photoFullUrlSmall.length != 0)
    {
        TGRemoteImageView *avatarView = [self.userInfoItem visibleAvatarView];
        
        NSString *legacyThumbnailUrl = user.photoFullUrlSmall.length != 0 ? user.photoFullUrlSmall : user.photoUrlSmall;
        NSString *legacyUrl = user.photoFullUrlBig.length != 0 ? user.photoFullUrlBig : user.photoUrlBig;
        if (legacyUrl.length == 0)
            legacyUrl = legacyThumbnailUrl;
        if (legacyThumbnailUrl.length == 0)
            legacyThumbnailUrl = legacyUrl;
        
        while (false) TGLog(@"IOS6PROFILE avatar.gallery.open uid=%d small=%@ big=%@ fullSmall=%@ fullBig=%@ chosenThumb=%@ chosen=%@ hasImage=%d",
              (int)_uid,
              user.photoUrlSmall,
              user.photoUrlBig,
              user.photoFullUrlSmall,
              user.photoFullUrlBig,
              legacyThumbnailUrl,
              legacyUrl,
              avatarView.currentImage != nil ? 1 : 0);
        
        if (user != nil && legacyUrl.length != 0 && avatarView.currentImage != nil)
        {
            ...
            modernGallery.model = [[TGUserAvatarGalleryModel alloc] initWithPeerId:_uid currentAvatarLegacyThumbnailImageUri:legacyThumbnailUrl currentAvatarLegacyImageUri:legacyUrl currentAvatarImageSize:CGSizeMake(640.0f, 640.0f)];
