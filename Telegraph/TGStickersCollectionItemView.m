...
#import "../submodules/LegacyComponents/LegacyComponents/TGMenuSheetCollectionView.h"
...
- (void)setStickerPack:(TGStickerPack *)stickerPack animated:(bool)animated
{
    ...
    NSMutableArray *titleLinks = [[NSMutableArray alloc] init];
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    if ([action isEqualToString:@"menuAction"])
    {
        ...
        if ([menuAction isEqualToString:@"send"])
        {
            ...
            if (index < 0 || index >= (NSInteger)_stickerPack.documents.count)
            {
                IOS6_NOOP_LOG(@"IOS6STICKER menu.send invalidIndex=%d count=%d", (int)index, (int)_stickerPack.documents.count);
                return;
            }
            TGDocumentMediaAttachment *sticker = _stickerPack.documents[index];
            IOS6_NOOP_LOG(@"IOS6STICKER menu.send index=%d doc=%lld access=%lld dc=%d size=%d mime=%@", (int)index, (long long)sticker.documentId, (long long)sticker.accessHash, sticker.datacenterId, sticker.size, sticker.mimeType);
            if (self.sendSticker != nil)
        ...
        else if ([menuAction isEqualToString:@"faveSticker"] || [menuAction isEqualToString:@"unfaveSticker"])
        {
            ...
            if (index < 0 || index >= (NSInteger)_stickerPack.documents.count)
            {
                IOS6_NOOP_LOG(@"IOS6STICKER menu.fave invalidIndex=%d count=%d action=%@", (int)index, (int)_stickerPack.documents.count, menuAction);
                return;
            }
            TGDocumentMediaAttachment *sticker = _stickerPack.documents[index];
