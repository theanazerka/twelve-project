...
+ (TGMenuSheetController *)presentInParentController:(TGViewController *)parentController menuController:(TGMenuSheetController *)menuController title:(NSString *)title webPageAttachment:(TGWebPageMediaAttachment *)webPage locationAttachment:(TGLocationMediaAttachment *)location directions:(bool)directions buttonTitle:(NSString *)buttonTitle buttonAction:(void (^)(TGMenuSheetController *))buttonAction sourceView:(UIView *)sourceView sourceRect:(CGRect (^)(void))sourceRect barButtonItem:(UIBarButtonItem *)barButtonItem
{
    ...
    {
        // UICollectionViewFlowLayout can produce an NSNotFound section while
        // the menu is being presented on iOS 6. Plain menu rows are lighter
        // and avoid that UIKit crash entirely.
        if (title.length != 0)
            [itemViews addObject:[[TGMenuSheetTitleItemView alloc] initWithTitle:title subtitle:nil]];
        for (TGOpenInAppItem *appItem in appItems)
        {
            TGMenuSheetButtonItemView *appButton = [[TGMenuSheetButtonItemView alloc] initWithTitle:appItem.title type:TGMenuSheetButtonTypeDefault action:^
            {
                __strong TGMenuSheetController *strongController = weakController;
                [strongController dismissAnimated:true manual:true];
                [appItem performOpenIn];
            }];
            [itemViews addObject:appButton];
        }
    }
    else
    {
