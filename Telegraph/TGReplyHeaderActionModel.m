...
+ (NSString *)messageTextForActionMedia:(TGActionMediaAttachment *)actionMedia otherAttachments:(NSArray *)__unused otherAttachments author:(id)author
{
    ...
    switch (actionMedia.actionType)
    {
        ...
        {
            NSString *formatString = [actionMedia.actionData[@"joinedByRequest"] boolValue] ? TGLocalized(@"Notification.JoinedChat") : TGLocalized(@"Notification.JoinedGroupByLink");
            messageText = [[NSString alloc] initWithFormat:formatString, [self titleForPeer:author shortName:false]];
