...
static bool TGIOS6RecentStickerDocumentIsAnimated(TGDocumentMediaAttachment *document)
{
...
static NSArray *TGIOS6RecentStaticStickerDocuments(NSArray *documents)
{
    ...
    {
        if (TGIOS6RecentStickerDocumentIsAnimated(document))
        {
            skipped++;
            continue;
        }
    ...
    if (skipped != 0)
        IOS6_NOOP_LOG(@"IOS6STICKER recent.filterAnimated skipped=%d kept=%d", (int)skipped, (int)result.count);
    return result;
}
    ...
    return [[[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber) {
        ...
        if (data == nil) {
            ...
            if (object == nil) {
                ...
                {
                    NSDictionary *dict = @{@"documents": TGIOS6RecentStaticStickerDocuments(object)};
                    [subscriber putNext:dict];
                }
                else if ([object isKindOfClass:[NSDictionary class]])
                {
                    NSMutableDictionary *dict = [[NSMutableDictionary alloc] initWithDictionary:object];
                    dict[@"documents"] = TGIOS6RecentStaticStickerDocuments(dict[@"documents"]);
                    object = dict;
...
+ (SSignal *)_remoteRecentStickers:(int32_t)hash {
    IOS6_NOOP_LOG(@"IOS6STICKER recent.remote start hash=%d", hash);
    ...
    return [[[TGTelegramNetworking instance] requestSignal:getRecentStickers] mapToSignal:^SSignal *(id result) {
        IOS6_NOOP_LOG(@"IOS6STICKER recent.remote result=%@", NSStringFromClass([result class]));
        if ([result isKindOfClass:[TLmessages_RecentStickers$messages_recentStickers class]]) {
            ...
            IOS6_NOOP_LOG(@"IOS6STICKER recent.remote stickers=%d dates=%d hash=%d", (int)recentStickers.stickers.count, (int)recentStickers.dates.count, recentStickers.n_hash);
            ...
            [recentStickers.stickers enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id desc, NSUInteger index, __unused BOOL *stop) {
                int32_t date = index < recentStickers.dates.count ? [recentStickers.dates[index] intValue] : 0;
                TGDocumentMediaAttachment *document = [[TGDocumentMediaAttachment alloc] initWithTelegraphDocumentDesc:desc];
                if (document.documentId != 0 && !TGIOS6RecentStickerDocumentIsAnimated(document)) {
...
+ (void)addRemoteRecentStickerFromDocuments:(NSArray *)addedDocuments sync:(bool)sync {
    ...
    if (sync) {
        for (TGDocumentMediaAttachment *addedDocument in addedDocuments) {
            if (TGIOS6RecentStickerDocumentIsAnimated(addedDocument))
                continue;
