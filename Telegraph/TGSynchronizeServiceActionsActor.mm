                ...
                if ([action isKindOfClass:[TGChangeNotificationSettingsFutureAction class]])
                {
                    ...
                    if (TGPeerIdIsChannel(notificationSettingsAction.uniqueId)) {
                        [TGDatabaseInstance() dispatchOnDatabaseThread:^{
                            ...
                            NSData *preservedAccessHashData = [TGDatabaseInstance() conversationCustomPropertySync:notificationSettingsAction.uniqueId name:murMurHash32(@"ios6NotificationAccessHash")];
                            if (preservedAccessHashData.length == sizeof(int64_t))
                            {
                                int64_t preservedAccessHash = 0;
                                [preservedAccessHashData getBytes:&preservedAccessHash length:sizeof(preservedAccessHash)];
                                if (preservedAccessHash != 0)
                                    accessHash = preservedAccessHash;
                            }
                        } synchronous:true];
                    }

                    TGLog(@"IOS6MUTE synchronize peer=%lld channel=%d accessHash=%lld muteUntil=%@", notificationSettingsAction.uniqueId, TGPeerIdIsChannel(notificationSettingsAction.uniqueId) ? 1 : 0, accessHash, notificationSettingsAction.muteUntil);
...
{
    TGLog(@"IOS6MUTE synchronize success peer=%lld", _currentActionUniqueId);
...
{
    TGLog(@"IOS6MUTE synchronize failed peer=%lld", _currentActionUniqueId);
    [TGDatabaseInstance() removeFutureAction:_currentActionUniqueId type:_currentActionType randomId:_currentActionRandomId];
    [self execute:nil];
}
