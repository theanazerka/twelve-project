...
- (int32_t)calculatedChannelsHash
{
    ...
    for (NSNumber *peerId in channelIds) {
        uint32_t channelId = TGChannelIdFromPeerId(peerId.longLongValue);
...
- (void)encodeInt64Array:(NSArray *)array coder:(PSKeyValueCoder *)coder key:(NSString *)key
{
    ...
    {
        [coder encodeInt64:number.longLongValue forKey:[NSString stringWithFormat:@"%@_%d", key, (int32_t)i]];
        i++;
    }
}
