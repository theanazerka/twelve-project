...
- (SSignal *)requestCallWithPeerId:(int64_t)peerId {
    TGLog(@"IOS6CALL manager.request peer=%lld", peerId);
...
- (SSignal *)requestCallWithPeerId:(int64_t)peerId uuid:(NSUUID *)uuid session:(TGCallSession *)session {
    return [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber) {
        ...
        [_queue dispatch:^{
            ...
            _nextInternalId += 1;
            TGLog(@"IOS6CALL manager.create peer=%lld internal=%d uuid=%@", peerId, internalId, uuid);
            ...
            [context.disposable setDisposable:[[[TGCallSignals requestedOutgoingCallWithPeerId:peerId] deliverOn:_queue] startWithNext:^(id next) {
                TGLog(@"IOS6CALL manager.request.next peer=%lld internal=%d context=%@", peerId, internalId, next);
                __strong TGCallManager *strongSelf = weakSelf;
                if (strongSelf != nil) {
                ...
                TGLog(@"IOS6CALL manager.request.error peer=%lld internal=%d error=%@ rpc=%@", peerId, internalId, error, rpcError);
                __strong TGCallManager *strongSelf = weakSelf;
                if (strongSelf != nil) {
                    NSString *rpcError = nil;
                    if ([error isKindOfClass:[MTRpcError class]])
...
- (void)updateCallContextWithCallId:(int64_t)callId callContext:(id)callContext {
    [_queue dispatch:^{
        ...
        if (internalId == -1 && [callContext isKindOfClass:[TGCallAcceptedContext class]]) {
            TGCallAcceptedContext *acceptedContext = (TGCallAcceptedContext *)callContext;
            [_callContexts enumerateKeysAndObjectsUsingBlock:^(NSNumber *key, TGCallContext *context, BOOL *stop) {
                if ([context.context isKindOfClass:[TGCallWaitingContext class]]) {
                    TGCallWaitingContext *waitingContext = (TGCallWaitingContext *)context.context;
                    if (waitingContext.callId == 0 && (waitingContext.participantId == acceptedContext.participantId || context.state.peerId == acceptedContext.participantId)) {
                        TGLog(@"IOS6CALL manager.attachAccepted call=%lld peer=%d internal=%d", acceptedContext.callId, acceptedContext.participantId, [key intValue]);
                        internalId = [key intValue];
                        *stop = true;
                    }
                }
        ...
        if (internalId != -1) {
            ...
            TGLog(@"IOS6CALL manager.incomingRequested call=%lld admin=%d participant=%d", requestedContext.callId, requestedContext.adminId, requestedContext.participantId);
...
- (void)handleSignalingDataForCallId:(int64_t)callId data:(NSData *)data {
    [_queue dispatch:^{
        __block NSNumber *matchedInternalId = nil;
        __block TGCallContext *matchedContext = nil;
        [_callContexts enumerateKeysAndObjectsUsingBlock:^(NSNumber *key, TGCallContext *context, BOOL *stop) {
            if ([context.context conformsToProtocol:@protocol(TGCallIdentifiableContext)]) {
                id<TGCallIdentifiableContext> identifiableContext = context.context;
                if (identifiableContext.callId == callId) {
                    matchedInternalId = key;
        ...
        if (matchedContext.session != nil) {
            TGLog(@"IOS6CALL signaling.route call=%lld internal=%d dataLen=%d", callId, [matchedInternalId intValue], (int)data.length);
            [matchedContext.session receiveSignalingData:data];
        } else {
            TGLog(@"IOS6CALL signaling.route.miss call=%lld dataLen=%d contexts=%d", callId, (int)data.length, (int)_callContexts.count);
        }
    }];
}
    ...
    if (context.context == nil) {
        ...
        if ([toCallContext isKindOfClass:[TGCallConfirmedContext class]]) {
            ...
            if (keyId == confirmedContext.keyFingerprint && [gAHash isEqual:previousContext.gAHash]) {
                ...
                context.context = [[TGCallOngoingContext alloc] initWithCallId:confirmedContext.callId accessHash:confirmedContext.accessHash date:confirmedContext.date adminId:confirmedContext.adminId participantId:confirmedContext.participantId key:key keyFingerprint:keyId defaultConnection:confirmedContext.defaultConnection alternativeConnections:confirmedContext.alternativeConnections webrtcConnections:confirmedContext.webrtcConnections customParameters:confirmedContext.customParameters];
                [self setContextState:internalId state:[[TGCallStateData alloc] initWithInternalId:@(internalId) callId:confirmedContext.callId accessHash:confirmedContext.accessHash state:TGCallStateOngoing peerId:confirmedContext.participantId connection:[[TGCallConnection alloc] initWithKey:key keyHash:visKeyHash defaultConnection:confirmedContext.defaultConnection alternativeConnections:confirmedContext.alternativeConnections webrtcConnections:confirmedContext.webrtcConnections customParameters:confirmedContext.customParameters] hungUpOutside:false needsRating:false needsDebug:false error:nil]];
            } else {
        ...
        if ([toCallContext isKindOfClass:[TGCallOngoingContext class]]) {
            ...
            [self setContextState:internalId state:[[TGCallStateData alloc] initWithInternalId:@(internalId) callId:ongoingContext.callId accessHash:ongoingContext.accessHash state:TGCallStateOngoing peerId:ongoingContext.participantId connection:[[TGCallConnection alloc] initWithKey:ongoingContext.key keyHash:visKeyHash defaultConnection:ongoingContext.defaultConnection alternativeConnections:ongoingContext.alternativeConnections webrtcConnections:ongoingContext.webrtcConnections customParameters:ongoingContext.customParameters] hungUpOutside:false needsRating:false needsDebug:false error:nil]];
        }
...
- (void)acceptCallWithInternalId:(id)internalId {
    ...
    [[[[[self callStateWithInternalId:internalId] mapToSignal:^SSignal *(TGCallStateData *state) {
        ...
        int32_t internalIdVal = [internalId intValue];
        [_queue dispatch:^{
            TGCallContext *context = _callContexts[internalId];
            __weak TGCallManager *weakSelf = self;
