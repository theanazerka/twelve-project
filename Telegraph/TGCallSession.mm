...
- (bool)startWithState:(TGCallStateData *)state outgoing:(bool)outgoing logPath:(NSString *)logPath statsLogPath:(NSString *)statsLogPath signalBarsChanged:(__unused void (^)(int32_t))signalBarsChanged stateChanged:(__unused void (^)(TGCallTransmissionState))stateChanged
{
    ...
    if (contextClass == nil || connectionClass == nil) {
        TGLog(@"IOS6CALL webrtc.runtime.missing contextClass=%@ connectionClass=%@", contextClass, connectionClass);
        return false;
    }
    
    NSArray *supportedVersions = nil;
    SEL versionsSelector = NSSelectorFromString(@"versionsWithIncludeReference:");
    if ([contextClass respondsToSelector:versionsSelector])
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        supportedVersions = [contextClass performSelector:versionsSelector withObject:nil];
#pragma clang diagnostic pop
    NSString *version = supportedVersions.count == 0 ? @"5.0.0" : [supportedVersions objectAtIndex:0];
    ...
    if (connections.count == 0) {
        TGLog(@"IOS6CALL webrtc.runtime.noConnections call=%lld", state.callId);
        return false;
    }
    ...
    void (^sendSignalingData)(NSData *) = ^(NSData *data) {
        TGLog(@"IOS6CALL webrtc.signaling.emit call=%lld dataLen=%d", state.callId, (int)data.length);
        [[TGCallSignals sendSignalingDataForCallId:state.callId accessHash:state.accessHash data:data] startWithNext:nil error:^(id error) {
            TGLog(@"IOS6CALL webrtc.signaling.send.error call=%lld error=%@", state.callId, error);
        } completed:nil];
    };
    
    NSString *customParameters = state.connection.customParameters;
    if (customParameters.length == 0)
        customParameters = TGIOS6CurrentCallsConfig;
    if (customParameters.length == 0)
        customParameters = @"{}";
    TGLog(@"IOS6CALL webrtc.runtime.customParametersFallback call=%lld stateLen=%d finalLen=%d", state.callId, (int)state.connection.customParameters.length, (int)customParameters.length);
    ...
    if (context == nil) {
        TGLog(@"IOS6CALL webrtc.runtime.initNil call=%lld version=%@", state.callId, version);
        return false;
    }
    
    self.context = context;
    
    TGLog(@"IOS6CALL webrtc.runtime.callbacks.disabled call=%lld reason=ios6-block-kvc-crash", state.callId);
    
    TGLog(@"IOS6CALL webrtc.runtime.start call=%lld version=%@ connections=%d customParametersLen=%d", state.callId, version, (int)connections.count, (int)customParameters.length);
    _started = true;
    return true;
}
...
- (void)receiveSignalingData:(NSData *)data
{
    ...
    TGLog(@"IOS6CALL webrtc.signaling.receive.forward dataLen=%d", (int)data.length);
...
- (void)receiveSignalingData:(NSData *)data
{
    ...
    TGLog(@"IOS6CALL signaling.session.receive call=%lld access=%lld packet=%d dataLen=%d controller=%d", _callId, _accessHash, (int)_receivedSignalingPackets, (int)data.length, _controller != nil ? 1 : 0);
    [_webrtcRuntime receiveSignalingData:data];
}
    ...
    TGDispatchOnMainThread(^
    {
        ...
        if (tranmissionState == TGCallTransmissionStateReconnecting)
        {
            if (_webrtcRuntime.started)
                TGLog(@"IOS6CALL webrtc.skipTone reconnecting");
            else
...
- (void)startTransmissionIfNeeded:(TGCallStateData *)state
{
    ...
    void (^block)(void) = ^
    {
        if (state.connection.webrtcConnections.count != 0)
            TGLog(@"IOS6CALL webrtc.skipTone connecting");
        else
            [self playTone:TGCallToneConnecting];
        ...
        }] timeout:10.5 onQueue:[SQueue mainQueue] orSignal:[SSignal single:@true]] take:1] startWithNext:^(__unused id next)
        {
            ...
            if (state.connection.webrtcConnections.count != 0) {
                TGLog(@"IOS6CALL webrtc.only.prepare call=%lld endpoints=%d customParameters=%@", state.callId, (int)state.connection.webrtcConnections.count, state.connection.customParameters);
                for (TGCallWebrtcConnectionDescription *connection in state.connection.webrtcConnections) {
                    TGLog(@"IOS6CALL webrtc.endpoint id=%lld flags=%d turn=%d stun=%d ip=%@ ipv6=%@ port=%d user=%@ passLen=%d",
                ...
                bool runtimeStarted = [_webrtcRuntime startWithState:state outgoing:_outgoing logPath:logPath statsLogPath:statsLogPath signalBarsChanged:^(int32_t bars) {
                    [self signalBarsChanged:bars];
                } stateChanged:^(TGCallTransmissionState transmissionState) {
                    [self webrtcTransmissionStateChanged:transmissionState];
                }];
                
                if (runtimeStarted) {
                    TGLog(@"IOS6CALL webrtc.runtime.active call=%lld legacyControllerSkipped=1", state.callId);
                    if (self.onStartedConnecting != nil)
                        self.onStartedConnecting();
                    return;
                }
                
                TGLog(@"IOS6CALL webrtc.engine.missing call=%lld storedWebRtc=%d usingLegacyLibtgvoip=0 signalingApiReady=1 hardBlock=1", state.callId, (int)state.connection.webrtcConnections.count);
                [self _discardCurrentCallWithReason:TGCallDiscardReasonDisconnect];
                return;
            }
            ...
            [_controller with:^id(VoIPControllerHolder *controller) {
                ...
                TGLog(@"IOS6CALL voip.start.prepare call=%lld default=%@ alt=%d total=%d webrtc=%d customParametersLen=%d", state.callId, state.connection.defaultConnection, (int)state.connection.alternativeConnections.count, (int)connections.count, (int)state.connection.webrtcConnections.count, (int)state.connection.customParameters.length);
                if (state.connection.webrtcConnections.count != 0) {
                    TGLog(@"IOS6CALL webrtc.engine.prepare call=%lld endpoints=%d customParameters=%@", state.callId, (int)state.connection.webrtcConnections.count, state.connection.customParameters);
                    for (TGCallWebrtcConnectionDescription *connection in state.connection.webrtcConnections) {
                        TGLog(@"IOS6CALL webrtc.endpoint id=%lld flags=%d turn=%d stun=%d ip=%@ ipv6=%@ port=%d user=%@ passLen=%d",
                              connection.identifier,
                              connection.flags,
                ...
                if (state.connection.webrtcConnections.count != 0) {
                    ...
                    if (runtimeStarted) {
                        TGLog(@"IOS6CALL webrtc.runtime.active call=%lld legacySkipped=1", state.callId);
                ...
                for (TGCallConnectionDescription *connection in connections) {
                    if (connection.ipv4.length == 0 || connection.ipv6.length == 0 || connection.peerTag.length < 16) {
                        TGLog(@"IOS6CALL voip.start.skipEndpoint id=%lld ipv4=%@ ipv6=%@ port=%d peerTagLen=%d", connection.identifier, connection.ipv4, connection.ipv6, connection.port, (int)connection.peerTag.length);
                        continue;
                    }
                ...
                TGLog(@"IOS6CALL voip.start.endpoints call=%lld count=%d", state.callId, (int)endpoints.size());
                if (endpoints.empty()) {
                    TGLog(@"IOS6CALL voip.start.block no endpoints call=%lld connection=%@", state.callId, state.connection);
                    return nil;
                }
                ...
                TGLog(@"IOS6CALL voip.allowP2P call=%lld peer=%lld mode=%d allow=%d forced=0", state.callId, state.peerId, p2pMode, allowP2P ? 1 : 0);
                
                controller.controller->SetConfig(config);

                controller.controller->SetEncryptionKey((char *)state.connection.key.bytes, _outgoing);
                controller.controller->SetRemoteEndpoints(endpoints, allowP2P, TGCallLegacyMaxLayer);
...
- (void)stopTransmission:(bool)sendDebugLog
{
    if (_webrtcRuntime.started) {
        ...
        [runtime stopWithCompletion:^(NSString *debugLog, int64_t bytesSentWifi, int64_t bytesReceivedWifi, int64_t bytesSentMobile, int64_t bytesReceivedMobile) {
            ...
            if (sendDebugLog && self.peerId != 0 && self.accessHash != 0 && debugLog.length != 0)
