...
#import "../submodules/LegacyComponents/LegacyComponents/TGModernGalleryEmbeddedStickersHeaderView.h"
        ...
        if (controller != nil) {
            ...
            if (stickerPackReferences.count == 1) {
                ...
                [_stickersInfoDisposable setDisposable:[[[[[_stickersInfo signal] filter:^bool(id next) {
                    return next != nil;
                }] take:1] deliverOn:[SQueue mainQueue]] startWithNext:^(NSArray *stickerPacks) {
                    __strong TGModernGalleryMessageImageItemView *strongSelf = weakSelf;
                    if (strongSelf != nil) {
                        if (stickerPacks.count == 1) {
                            TGViewController *controller = [strongSelf.delegate parentControllerForPresentation];
                            TGStickerPack *stickerPack = (TGStickerPack *)stickerPacks[0];
                            [TGStickersMenu presentInParentController:controller stickerPack:stickerPack showShareAction:false sendSticker:nil stickerPackRemoved:nil stickerPackHidden:nil stickerPackArchived:false stickerPackIsMask:stickerPack.isMask sourceView:strongSelf->_stickersHeaderView sourceRect:^CGRect{
                                return sourceRect;
                            }];
...
- (void)previewMultipleStickerPacks:(NSArray *)stickerPacks {
