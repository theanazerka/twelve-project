...
+ (SSignal *)shareItemForAudioVideoMessage:(TGMessage *)message {
    ...
    return [[[TGMusicPlayerItemSignals itemAvailability:item priority:true] filter:^bool(id value) {
        TGMusicPlayerItemAvailability availability = TGMusicPlayerItemAvailabilityUnpack([value longLongValue]);
...
+ (SSignal *)shareItemForContactMessage:(TGMessage *)message {
    ...
    if (iosMajorVersion() >= 9)
    {
        ...
        {
            CNLabeledValue *phoneValue = [CNLabeledValue labeledValueWithLabel:nil value:[CNPhoneNumber phoneNumberWithStringValue:phoneNumber]];
