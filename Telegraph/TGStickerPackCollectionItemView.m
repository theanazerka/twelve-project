...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        _altLabel.font = [UIFont systemFontOfSize:24.0f];
        _altLabel.hidden = true;
        [self.editingContentView addSubview:_altLabel];
...
- (void)setPresentation:(TGPresentation *)presentation
{
    ...
    _altLabel.textColor = presentation.pallete.collectionMenuTextColor;
...
- (void)setStickerPack:(TGStickerPack *)stickerPack
{
    ...
    if (stickerPack.documents.count != 0)
    {
        ...
        bool showAltFallback = alt.length != 0 && ([documentMedia.mimeType isEqualToString:@"video/webm"] || [documentMedia.mimeType isEqualToString:@"application/x-tgsticker"]);
        if (showAltFallback)
        {
            [_imageView reset];
            _imageView.hidden = true;
            _altLabel.hidden = false;
            _altLabel.text = alt;
            IOS6_NOOP_LOG(@"IOS6STICKER packCell.altFallback title=%@ doc=%lld mime=%@ alt=%@", title, (long long)documentMedia.documentId, documentMedia.mimeType, alt);
        }
        else
        {
            _imageView.hidden = false;
...
- (void)layoutSubviews
{
    ...
    _altLabel.frame = _imageView.frame;
