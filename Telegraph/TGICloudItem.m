...
+ (instancetype)iCloudItemWithUrl:(NSURL *)url fileSize:(NSUInteger)fileSize
{
    ...
    item->_fileUrl = url;
    item->_fileName = [url.absoluteString.lastPathComponent stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
    fileAttributes = [url resourceValuesForKeys:@[ NSURLUbiquitousItemDownloadingStatusKey ] error:&error];
#endif
...
#else
    [request->_query setSearchScopes:@[ NSMetadataQueryUbiquitousDocumentsScope ]];
#endif
