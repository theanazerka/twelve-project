        ...
        SSignal *(^convertSignal)(NSURL *) = ^(NSURL *url)
        {
            return [[server server] mapToSignal:^SSignal *(TGBridgeServer *server) {
                return [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber)
                {
                    ...
                    {
                        NSURL *finalURL = TGFileURLWithPathRelativeToURL([NSString stringWithFormat:@"%@.%@", result.lastPathComponent, @"m4a"], server.temporaryFilesURL);
...
+ (void)handleIncomingAudioWithURL:(NSURL *)url metadata:(NSDictionary *)metadata server:(TGBridgeServer *)server
{
    ...
    int64_t uniqueId = [metadata[TGBridgeIncomingFileRandomIdKey] longLongValue];
    int64_t peerId = [metadata[TGBridgeIncomingFilePeerIdKey] longLongValue];
    int32_t replyToMid = [metadata[TGBridgeIncomingFileReplyToMidKey] intValue];
    ...
    [[[server server] onNext:^(TGBridgeServer *server) {
        [server startSignalForKey:signalKey producer:^SSignal *{
            ...
            {
                return [[TGSendAudioSignal sendAudioWithPeerId:peerId tempDataItem:result[@"dataItem"] liveData:result[@"liveData"] duration:[result[@"duration"] intValue] localAudioId:uniqueId replyToMid:replyToMid] onNext:^(TGMessage *next)
                {
