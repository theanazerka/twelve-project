...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (isCurrent) {
        ...
        if ([((TGAvailableLocalization *)_filteredItems[indexPath.row]).code isEqualToString:@"custom"]) {
            if (_applyingLocalizationCode != nil) {
                _applyingLocalizationCode = nil;
                [_disposable setDisposable:nil];
            }
            ...
            _applyingLocalizationCode = ((TGAvailableLocalization *)_filteredItems[indexPath.row]).code;
            __weak TGLocalizationSelectionController *weakSelf = self;
