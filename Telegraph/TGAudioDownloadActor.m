    ...
    if (audioAttachment != nil)
    {
       ...
       // NSDictionary *attributes = [[NSFileManager defaultManager] attributesOfItemAtPath:audioFilePath error:nil];
        ...
        else
        {
            ...
            inputDocumentLocation.file_reference = [audioAttachment.ios6OriginInfo fileReference] ?: [NSData data];
            inputFileLocation = inputDocumentLocation;
            
            datacenterId = audioAttachment.datacenterId;
            while (false) TGLog(@"IOS6MEDIA audio location id=%lld dc=%d size=%d fileRef=%d origin=%d", audioAttachment.audioId, audioAttachment.datacenterId, audioAttachment.fileSize, (int)inputDocumentLocation.file_reference.length, audioAttachment.ios6OriginInfo != nil ? 1 : 0);
        }
        ...
        if (inputFileLocation != nil)
        {
            ...
            if (audioAttachment.ios6OriginInfo != nil)
                multipartOptions[@"originInfo"] = audioAttachment.ios6OriginInfo;
            if (audioAttachment.audioId != 0)
                multipartOptions[@"identifier"] = @(audioAttachment.audioId);
            
            [ActionStageInstance() requestActor:[[NSString alloc] initWithFormat:@"/tg/multipart-file/(audio:%" PRId64 ":%d:%@)", audioAttachment.audioId, audioAttachment.datacenterId, audioAttachment.audioUri.length != 0 ? audioAttachment.audioUri : @""] options:multipartOptions watcher:self];
        }
        else
