...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    for (NSNumber *peerId in peerIds) {
        [requests addObject:[[TGReadPeerMessagesRequest alloc] initWithPeerId:peerId.longLongValue maxMessageIndex:nil date:0 length:0 unread:false]];
    }
...
- (void)actionStageResourceDispatched:(NSString *)path resource:(id)resource arguments:(id)__unused arguments
{
    [_queue dispatch:^
    {
        ...
        else if ([path isEqualToString:@"/tg/messagesDeleted"])
        {
            ...
            int64_t peerId = [dictionary[@"peerId"] longLongValue];
