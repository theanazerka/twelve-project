...
#import "../submodules/LegacyComponents/LegacyComponents/TGLocationLiveSessionItemView.h"
...
static bool TGIOS6DialogListConversationIsArchived(TGConversation *conversation)
{
    return conversation.isArchived;
}

static NSSet *TGIOS6CompleteArchivePeerIds()
{
    NSData *completeData = [TGDatabaseInstance() customProperty:@"ios6ArchivePeerIdsComplete"];
    uint8_t complete = 0;
    if (completeData.length != 1)
        return nil;
    [completeData getBytes:&complete length:1];
    if (complete == 0)
        return nil;
    
    NSData *peerIdsData = [TGDatabaseInstance() customProperty:@"ios6ArchivePeerIds"];
    if (peerIdsData.length == 0)
        return [NSSet set];
    @try
    {
        NSArray *peerIds = [NSKeyedUnarchiver unarchiveObjectWithData:peerIdsData];
        if ([peerIds isKindOfClass:[NSArray class]])
            return [NSSet setWithArray:peerIds];
    }
...
static NSComparisonResult TGIOS6DialogListPinnedDateFirstCompare(id<TGDialogListItem> obj1, id<TGDialogListItem> obj2)
{
    ...
    int32_t pinnedDate1 = [obj1 isKindOfClass:[TGConversation class]] ? ((TGConversation *)obj1).pinnedDate : 0;
    int32_t pinnedDate2 = [obj2 isKindOfClass:[TGConversation class]] ? ((TGConversation *)obj2).pinnedDate : 0;
    if (pinnedDate1 != 0 || pinnedDate2 != 0) {
        if (pinnedDate1 == 0)
            return NSOrderedDescending;
        if (pinnedDate2 == 0)
            return NSOrderedAscending;
        if (pinnedDate1 > pinnedDate2)
            return NSOrderedAscending;
        if (pinnedDate1 < pinnedDate2)
            return NSOrderedDescending;
    }
    
    int date1 = (int)obj1.date;
    int date2 = (int)obj2.date;
    if (date1 > date2)
        return NSOrderedAscending;
    if (date1 < date2)
        return NSOrderedDescending;
    if (obj1.conversationId < obj2.conversationId)
        return NSOrderedDescending;
    if (obj1.conversationId > obj2.conversationId)
        return NSOrderedAscending;
...
@property (nonatomic, strong) NSMutableArray *archiveConversationList;
...
- (id)init
{
    ...
    if (self != nil)
    {
        ...
        _archiveConversationList = [[NSMutableArray alloc] init];
...
//        {
//            int32_t totalUnreadCount = [total intValue];
//            int32_t visibleUnreadCount = [visible intValue];
...
- (void)clearData
{
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        [_archiveConversationList removeAllObjects];
        _adConversation = nil;
...
{    
    IOS6Trace(@"IOS6FULL ui.dialog.select peer=%lld title=%@ unread=%d serviceUnread=%d date=%d kind=%d channel=%d group=%d broadcast=%d access=%lld forward=%d privacy=%d privateOnly=%d groupsChannelsOnly=%d", conversation.conversationId, conversation.chatTitle, conversation.unreadCount, conversation.serviceUnreadCount, conversation.date, (int)conversation.kind, conversation.isChannel ? 1 : 0, conversation.isChannelGroup ? 1 : 0, conversation.isBroadcast ? 1 : 0, conversation.accessHash, self.forwardMode ? 1 : 0, self.privacyMode ? 1 : 0, self.showPrivateOnly ? 1 : 0, self.showGroupsAndChannelsOnly ? 1 : 0);
...
{
    IOS6Trace(@"IOS6FULL ui.search.selectConversation peer=%lld title=%@ access=%lld", conversation.conversationId, conversation.chatTitle, conversation.accessHash);
...
{
    IOS6Trace(@"IOS6FULL ui.search.selectMessage peer=%lld title=%@ mid=%d access=%lld", conversation.conversationId, conversation.chatTitle, messageId, conversation.accessHash);
...
- (void)scrollToNextUnreadChat {
    [[self.dialogListController.atTopSignal take:1] startWithNext:^(NSNumber *atTop)
    {
        if (atTop.boolValue)
        {
            [TGDatabaseInstance() loadUnreadConversationListFromDate:0 limit:400 completion:^(NSArray *result) {
                TGDispatchOnMainThread(^{
                    int64_t earliestUnreadConversationId = ((TGConversation *)result.firstObject).conversationId;
        ...
        else
        {
            ...
            [TGDatabaseInstance() loadUnreadConversationListFromDate:conversation.date limit:1 completion:^(NSArray *result) {
                TGDispatchOnMainThread(^{
                    int64_t nextUnreadConversationId = ((TGConversation *)result.firstObject).conversationId;
...
- (void)clearChatListCacheAndReload
{
    TGLog(@"IOS6ARCHIVE cache.button companion reload");
    IOS6Trace(@"IOS6FULL ui.dialog.cacheButton.reload current=%d", (int)_conversationList.count);
    ...
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^
        {
            ...
            if (controller != nil)
            {
                ...
                [controller dialogListFullyReloaded:@[]];
            }
        });
        
        [ActionStageInstance() requestActor:@"/tg/dialoglist/(0)" options:@{@"force": @true, @"limit": @50} watcher:self];
    }];
}
...
- (void)loadMoreItems:(int)limit
{
    if (limit == 0)
        limit = 30;
    IOS6Trace(@"IOS6FULL ui.dialog.loadMore.start limit=%d current=%d canLoadMore=%d", limit, (int)_conversationList.count, _canLoadMore ? 1 : 0);
    ...
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        if (minDate != INT_MAX)
        {
            IOS6Trace(@"IOS6FULL ui.dialog.loadMore.request minDate=%d currentIds=%d limit=%d", minDate, (int)currentConversationIds.count, limit);
        ...
        {
            _canLoadMore = false;
            IOS6Trace(@"IOS6FULL ui.dialog.loadMore.noMinDate current=%d", (int)_conversationList.count);
...
{
    IOS6Trace(@"IOS6FULL ui.dialog.archive.loadItems current=%d", (int)_conversationList.count);
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        [ActionStageInstance() requestActor:@"/tg/dialoglist/(archive)" options:@{@"folderId": @1, @"limit": @200} watcher:self];
    }];
}
...
- (void)initializeDialogListData:(TGConversation *)conversation customUser:(TGUser *)customUser selfUser:(TGUser *)selfUser
{
    ...
    bool ios6Archived = TGIOS6DialogListConversationIsArchived(conversation);
...
- (void)actorCompleted:(int)resultCode path:(NSString *)path result:(id)result
{
    ...
    {
        IOS6Trace(@"IOS6FULL ui.dialog.actorCompleted path=%@ status=%d result=%@", path, resultCode, NSStringFromClass([result class]));
        if (resultCode == 0)
        {
            bool ios6ArchiveListRequest = [path rangeOfString:@"archive"].location != NSNotFound;
            bool previousCanLoadMore = _canLoadMore;
            ...
            IOS6Trace(@"IOS6FULL ui.dialog.actorCompleted.items path=%@ loaded=%d before=%d", path, (int)loadedItems.count, (int)_conversationList.count);
            int rawLoadedItemsCount = (int)loadedItems.count;
            int ios6ExpectedPageSize = 1;
            if ([path isEqualToString:@"/tg/dialoglist/(0)"])
                ios6ExpectedPageSize = 50;
            else if (ios6ArchiveListRequest)
                ios6ExpectedPageSize = 200;
            ...
            if (forwardMode || privacyMode)
            {
                for (int i = 0; i < (int)loadedItems.count; i++)
                {
                    if (((id<TGDialogListItem>)loadedItems[i]).isBroadcast)
                    {
                        [loadedItems removeObjectAtIndex:i];
                        i--;
                    }
            ...
            for (int i = 0; i < (int)loadedItems.count; i++)
            {
                if (((id<TGDialogListItem>)loadedItems[i]).isDeactivated)
                {
                    [loadedItems removeObjectAtIndex:i];
                    i--;
                }
            ...
            for (int i = 0; i < (int)loadedItems.count; i++)
            {
                if (((id<TGDialogListItem>)loadedItems[i]).feedId.intValue != 0)
                {
                    [loadedItems removeObjectAtIndex:i];
                    i--;
                }
            ...
            if (!ios6ArchiveListRequest)
            {
                NSSet *completeArchivePeerIds = TGIOS6CompleteArchivePeerIds();
                for (NSInteger i = (NSInteger)loadedItems.count - 1; i >= 0; i--)
                {
                    id<TGDialogListItem> item = loadedItems[(NSUInteger)i];
                    if (![item isKindOfClass:[TGConversation class]])
                        continue;
                    TGConversation *conversation = (TGConversation *)item;
                    if (completeArchivePeerIds != nil)
                        conversation.isArchived = conversation.pinnedDate == 0 && [completeArchivePeerIds containsObject:@(conversation.conversationId)];
                    else if (conversation.pinnedDate != 0)
                        conversation.isArchived = false;
                    for (NSInteger archiveIndex = (NSInteger)_archiveConversationList.count - 1; archiveIndex >= 0; archiveIndex--)
                    {
                        TGConversation *archiveConversation = _archiveConversationList[(NSUInteger)archiveIndex];
                        if (archiveConversation.conversationId == conversation.conversationId)
                            [_archiveConversationList removeObjectAtIndex:(NSUInteger)archiveIndex];
                    }
                    if (conversation.isArchived)
                    {
                        [_archiveConversationList insertObject:conversation atIndex:0];
                        [loadedItems removeObjectAtIndex:(NSUInteger)i];
                    }
                }
            }
            ...
            if (ios6ArchiveListRequest)
            {
                NSMutableSet *seenArchivePeerIds = [[NSMutableSet alloc] init];
                NSMutableArray *archivePeerIds = [[NSMutableArray alloc] init];
                [_archiveConversationList removeAllObjects];
                for (id<TGDialogListItem> item in loadedItems)
                {
                    ...
                    if (![seenArchivePeerIds containsObject:peerId])
                    {
                        [seenArchivePeerIds addObject:peerId];
                        [archivePeerIds addObject:peerId];
                        [_archiveConversationList addObject:conversation];
                    }
                }

                NSMutableArray *combinedItems = [[NSMutableArray alloc] initWithArray:_conversationList];
                ...
                IOS6Trace(@"IOS6FULL ui.dialog.archive.separate main=%d archive=%d", (int)_conversationList.count, (int)_archiveConversationList.count);
                TGDispatchOnMainThread(^{
                    TGDialogListController *controller = self.dialogListController;
                    if (controller != nil)
                    {
                        controller.canLoadMore = previousCanLoadMore;
                        [controller dialogListFullyReloaded:combinedItems];
                    }
            ...
            {
                canLoadMore = rawLoadedItemsCount >= ios6ExpectedPageSize;
                if (_adConversation != nil) {
            ...
            IOS6Trace(@"IOS6FULL ui.dialog.actorCompleted.canLoadMore raw=%d expected=%d added=%d result=%d archive=%d", rawLoadedItemsCount, ios6ExpectedPageSize, (int)loadedItems.count, canLoadMore ? 1 : 0, ios6ArchiveListRequest ? 1 : 0);
            ...
            {
                return TGIOS6DialogListPinnedDateFirstCompare(conversation1, conversation2);
            }];
            ...
            NSMutableArray *items = [[NSMutableArray alloc] initWithArray:_conversationList];
            if (_archiveConversationList.count != 0)
                [items addObjectsFromArray:_archiveConversationList];
...
- (void)actionStageResourceDispatched:(NSString *)path resource:(id)resource arguments:(id)arguments
{
    ...
    else if ([path isEqualToString:@"/tg/conversations"] || [path isEqualToString:@"/tg/broadcastConversations"])
    {
        ...
        [_conversationList sortUsingComparator:^NSComparisonResult(id<TGDialogListItem> obj1, id<TGDialogListItem> obj2)
         {
             return TGIOS6DialogListPinnedDateFirstCompare(obj1, obj2);
         }];
