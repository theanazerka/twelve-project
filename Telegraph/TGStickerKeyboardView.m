...
#import "../submodules/LegacyComponents/LegacyComponents/TGStickerCollectionViewCell.h"
...
#import "../submodules/LegacyComponents/LegacyComponents/TGStickerItemPreviewView.h"
...
static NSString *TGIOS6StickerPackReferenceDescription(id reference)
{
    if ([reference isKindOfClass:[TGStickerPackIdReference class]])
    {
        TGStickerPackIdReference *idReference = (TGStickerPackIdReference *)reference;
        return [NSString stringWithFormat:@"id:%lld short:%@", (long long)idReference.packId, idReference.shortName];
    }
    if ([reference isKindOfClass:[TGStickerPackShortnameReference class]])
        return [NSString stringWithFormat:@"short:%@", ((TGStickerPackShortnameReference *)reference).shortName];
    return [NSString stringWithFormat:@"%@:%@", NSStringFromClass([reference class]), reference];
}

static NSString *TGIOS6StickerDocumentDescription(TGDocumentMediaAttachment *document)
{
    if (document == nil)
        return @"(nil)";
    return [NSString stringWithFormat:@"doc=%lld access=%lld dc=%d size=%d mime=%@ sticker=%d animated=%d pack={%@}",
        ...
        TGIOS6StickerPackReferenceDescription(document.stickerPackReference)];
}
...
- (void)setupSignals
{
    ...
    NSArray *keyboardSignals = @[
        (iosMajorVersion() >= 8 && _style == TGStickerKeyboardViewDefaultStyle) ? [TGRecentGifsSignal recentGifs] : [SSignal single:@[]],
        [[TGStickersSignals stickerPacks] startOn:[SQueue concurrentDefaultQueue]],
        [TGRecentStickersSignal recentStickers],
        [TGFavoriteStickersSignal favoriteStickers],
        groupStickersSignal,
        pinVar.signal
    ];
    SSignal *combinedSignal = [SSignal combineSignals:keyboardSignals withInitialStates:@[
        @[],
        @{@"packs": @[], @"featuredPacks": @[], @"featuredPacksUnreadIds": @[]},
        @{@"documents": @[], @"dates": @{}},
        @[],
        @{},
        @true
    ...
    _stickerPacksDisposable = [[combinedSignal deliverOn:[SQueue mainQueue]] startWithNext:^(NSArray *combinedResult)
    {
        ...
        NSArray *rawPacks = dict[@"packs"];
        NSArray *featuredPacks = dict[@"featuredPacks"];
        NSDictionary *recentResult = combinedResult[2];
        NSArray *recentDocuments = recentResult[@"documents"];
        NSArray *favoriteDocuments = combinedResult[3];
        IOS6_NOOP_LOG(@"IOS6STICKER keyboard.combined rawPacks=%d featured=%d recent=%d favorite=%d gifs=%d",
            (int)rawPacks.count,
            (int)featuredPacks.count,
            (int)recentDocuments.count,
            (int)favoriteDocuments.count,
            (int)gifs.count);
        ...
        for (id item in [combinedResult[3] reverseObjectEnumerator]) {
            ...
            if ([document.stickerPackReference isKindOfClass:[TGStickerPackIdReference class]]) {
                ...
                TGStickerPack *pack = packIdMap[@(packReference.packId)];
                if (pack != nil)
                    documentToStickerPack[@(document.documentId)] = pack;
            }
        }
        ...
        IOS6_NOOP_LOG(@"IOS6STICKER keyboard.filtered packs=%d recent=%d favorite=%d group=%d groupRef=%@",
            (int)reversed.count,
            (int)reversedRecentStickers.count,
            (int)reversedFavoriteStickers.count,
            (int)groupStickers.count,
            TGIOS6StickerPackReferenceDescription(groupStickerPackReference));
        ...
        if (strongSelf != nil) {
            ...
            if (strongSelf->_peerId != 0 && conversation != nil)
            {
            ...
            if (![strongSelf->_stickerPacks isEqual:reversed] || ![strongSelf->_recentStickersOriginal isEqual:reversedRecentStickers] || ![strongSelf->_favoriteDocuments isEqual:reversedFavoriteStickers] || ![strongSelf->_groupDocuments isEqual:groupStickers]) {
                ...
                IOS6_NOOP_LOG(@"IOS6STICKER keyboard.set packs=%d recent=%d favorite=%d group=%d peer=%lld admin=%d groupUnpinned=%d",
                    (int)reversed.count,
                    (int)updatedRecentStickers.count,
                    (int)reversedFavoriteStickers.count,
                    (int)groupStickers.count,
                    (long long)strongSelf->_peerId,
                    strongSelf->_isGroupAdmin ? 1 : 0,
                    strongSelf->_groupStickersUnpinned ? 1 : 0);
            ...
            if (![strongSelf->_trendingStickerPacks isEqualToArray:trendingPacks]) {
                IOS6_NOOP_LOG(@"IOS6STICKER keyboard.trending packs=%d installed=%d unread=%d", (int)trendingPacks.count, (int)installedPacks.count, (int)unreadPacks.count);
...
{
    if (indexPath == nil || indexPath.item >= _recentGifs.count)
        return nil;
...
- (TGStickerPack *)stickerPackAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0 || indexPath.section == 1 || indexPath.section == 2 || indexPath.section == [self lastGroupSection])
    {
        ...
        return document == nil ? nil : _recentDocumentToStickerPack[@(document.documentId)];
    }

    NSInteger packIndex = indexPath.section - 3;
    if (packIndex < 0 || packIndex >= (NSInteger)_stickerPacks.count)
        return nil;
    return _stickerPacks[(NSUInteger)packIndex];
}
...
- (TGDocumentMediaAttachment *)documentAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        if (indexPath.item >= _favoriteDocuments.count)
            return nil;
        document = _favoriteDocuments[indexPath.item];
    ...
    {
        if (indexPath.item >= _recentDocuments.count)
            return nil;
        document = _recentDocuments[indexPath.item];
        if (document.originInfo == nil)
    ...
    {
        if (indexPath.item >= _groupDocuments.count)
            return nil;
        document = _groupDocuments[indexPath.item];
    }
    else
    {
        NSInteger packIndex = indexPath.section - 3;
        if (packIndex < 0 || packIndex >= (NSInteger)_stickerPacks.count)
            return nil;
        TGStickerPack *pack = _stickerPacks[(NSUInteger)packIndex];
        if (indexPath.item >= pack.documents.count)
            return nil;
        document = pack.documents[indexPath.item];
        if (document.originInfo == nil)
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (collectionView == _gifsCollectionView) {
        ...
        if (cell != nil && _gifSelected) {
            TGDocumentMediaAttachment *document = [self gifAtIndexPath:indexPath];
            if (document == nil)
                return;
            IOS6_NOOP_LOG(@"IOS6STICKER keyboard.selectGif section=%d row=%d %@", (int)indexPath.section, (int)indexPath.row, TGIOS6StickerDocumentDescription(document));
            _gifSelected(document);
        }
    } else if (collectionView == _trendingCollectionView) {
    } else if (collectionView == _searchCollectionView) {
        ...
        if (_searchLocalStickerPacks.count > 0 && indexPath.section != (NSInteger)_searchLocalStickerPacks.count) {
            ...
            if ([cell isEnabled])
            {
                ...
                if (indexPath.section < 0 || indexPath.section >= (NSInteger)_searchLocalStickerPacks.count)
                    return;
                TGStickerPack *pack = _searchLocalStickerPacks[indexPath.section];
                if (indexPath.item >= pack.documents.count)
                    return;
                TGDocumentMediaAttachment *document = pack.documents[indexPath.item];
                if (document == nil)
                    return;
                IOS6_NOOP_LOG(@"IOS6STICKER keyboard.selectSearch section=%d row=%d %@", (int)indexPath.section, (int)indexPath.item, TGIOS6StickerDocumentDescription(document));
                if (_stickerSelected)
        ...
        if ([cell isEnabled])
        {
            ...
            if (document == nil)
                return;
            IOS6_NOOP_LOG(@"IOS6STICKER keyboard.select section=%d row=%d %@", (int)indexPath.section, (int)indexPath.item, TGIOS6StickerDocumentDescription(document));
            if (_stickerSelected)
                _stickerSelected(document);
        }
    }
...
- (void)installStickerPack:(TGStickerPack *)stickerPack {
    IOS6_NOOP_LOG(@"IOS6STICKER keyboard.installPack title=%@ docs=%d ref=%@",
        stickerPack.title,
        (int)stickerPack.documents.count,
        TGIOS6StickerPackReferenceDescription(stickerPack.packReference));
    ...
    [[[installStickerPackAndGetArchivedSignal deliverOn:[SQueue mainQueue]] onDispose:^{
        ...
        IOS6_NOOP_LOG(@"IOS6STICKER keyboard.installPack.done title=%@ archived=%d", stickerPack.title, (int)archivedPacks.count);
...
{
    IOS6_NOOP_LOG(@"IOS6STICKER keyboard.viewPack title=%@ docs=%d recent=%d sticker=%@",
        stickerPack.title,
        (int)stickerPack.documents.count,
        recent ? 1 : 0,
        TGIOS6StickerDocumentDescription(sticker));
...
- (void)previewStickerPack:(TGStickerPack *)stickerPack sticker:(TGDocumentMediaAttachment *)sticker {
    TGViewController *parentViewController = _parentViewController;
    IOS6_NOOP_LOG(@"IOS6STICKER keyboard.previewPack title=%@ docs=%d packRef=%@ sticker=%@ parent=%@",
        stickerPack.title,
        (int)stickerPack.documents.count,
        TGIOS6StickerPackReferenceDescription(stickerPack.packReference),
        TGIOS6StickerDocumentDescription(sticker),
    ...
    IOS6_NOOP_LOG(@"IOS6STICKER keyboard.previewPack.present menuPackRef=%@ expanded=%d search=%d", TGIOS6StickerPackReferenceDescription(packReference), _expanded ? 1 : 0, _searchActive ? 1 : 0);
    [TGStickersMenu presentWithParentController:parentViewController packReference:packReference stickerPack:stickerPack showShareAction:false sendSticker:^(TGDocumentMediaAttachment *document) {
        IOS6_NOOP_LOG(@"IOS6STICKER keyboard.previewPack.send %@", TGIOS6StickerDocumentDescription(document));
        __strong TGStickerKeyboardView *strongSelf = weakSelf;
        if (strongSelf != nil) {
