...
- (instancetype)initWithMethods:(NSArray *)methods selectedIndex:(NSUInteger)selectedIndex {
    self = [super init];
    if (self != nil) {
        _methods = methods;
...
+ (bool)supportsApplePay {
    if (iosMajorVersion() >= 8) {
        Class paymentAuthorizationClass = NSClassFromString(@"PKPaymentAuthorizationViewController");
        if ([paymentAuthorizationClass respondsToSelector:@selector(canMakePaymentsUsingNetworks:)]) {
            NSArray *networks = @[@"Visa", @"MasterCard", @"Amex"];
            return [(id)paymentAuthorizationClass canMakePaymentsUsingNetworks:networks];
        }
    }
