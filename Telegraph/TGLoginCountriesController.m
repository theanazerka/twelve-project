...
+ (NSString *)localizedCountryNameByCountryId:(NSString *)countryId code:(int *)code
{
    ...
    NSString *name = [locale displayNameForKey:NSLocaleCountryCode value:countryId];
...
- (NSArray *)localizedCountries:(NSArray *)countries
{
    ...
    for (NSArray *array in countries)
    {
        ...
        if (iosMajorVersion() >= 10 && ![[locale objectForKey:NSLocaleLanguageCode] isEqualToString:@"en"])
            localizedName = [locale displayNameForKey:NSLocaleCountryCode value:code];
