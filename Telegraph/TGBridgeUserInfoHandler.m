...
+ (SSignal *)handlingSignalForSubscription:(TGBridgeSubscription *)subscription server:(TGBridgeServer *)__unused server
{
    if ([subscription isKindOfClass:[TGBridgeUserInfoSubscription class]])
    {
        ...
        {
            SSignal *userSignal = [[TGUserSignal userWithUserId:userId.intValue] mapToSignal:^SSignal *(TGUser *user)
            {
    ...
    else if ([subscription isKindOfClass:[TGBridgeUserBotInfoSubscription class]])
    {
        ...
        {
            SSignal *botInfoSignal = [[TGBotSignals botInfoForUserId:userId.intValue] map:^TGBridgeBotInfo *(TGBotInfo *botInfo)
            {
                return [TGBridgeBotInfo botInfoWithTGBotInfo:botInfo userId:userId.intValue];
            }];
