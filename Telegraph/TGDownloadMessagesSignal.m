...
extern NSArray *TGIOS6ModernInputMessageIdsFromMessageIds(NSArray *messageIds);

static inline bool TGIOS6DownloadPeerIdIsModernRawChannel(int64_t peerId, int64_t accessHash)
{
    int64_t channelId = -peerId - 4294967296LL;
    return accessHash != 0 && channelId > 0 && channelId <= UINT32_MAX;
}

static inline bool TGIOS6DownloadPeerIdIsChannel(int64_t peerId, int64_t accessHash)
{
    return TGIOS6DownloadPeerIdIsModernRawChannel(peerId, accessHash) || TGPeerIdIsChannel(peerId);
}

static inline int64_t TGIOS6DownloadChannelIdFromPeerId(int64_t peerId, int64_t accessHash)
{
    if (TGIOS6DownloadPeerIdIsModernRawChannel(peerId, accessHash))
...
+ (SSignal *)downloadMessages:(NSArray *)messages
{
    ...
    for (TGDownloadMessage *message in messages) {
        if (TGIOS6DownloadPeerIdIsChannel(message.peerId, message.accessHash)) {
    ...
    if (channelMessageIdsByPeerId.count != 0) {
        ...
        [channelMessageIdsByPeerId enumerateKeysAndObjectsUsingBlock:^(__unused id key, NSArray *messages, __unused BOOL *stop) {
            if (messages.count != 0) {
                TGDownloadMessage *firstMessage = (TGDownloadMessage *)messages[0];
                ...
                inputChannel.channel_id = TGIOS6DownloadChannelIdFromPeerId(firstMessage.peerId, firstMessage.accessHash);
                inputChannel.access_hash = firstMessage.accessHash;
                ...
                getChannelMessages.n_id = TGIOS6ModernInputMessageIdsFromMessageIds(messageIds);
    ...
    if (genericMessageIds.count != 0) {
        ...
        getMessages.n_id = TGIOS6ModernInputMessageIdsFromMessageIds(genericMessageIds);
...
+ (SSignal *)mediaStickerpacks:(TGMediaAttachment *)attachment {
    IOS6_NOOP_LOG(@"IOS6MEDIA stickers disabled: skip attached sticker packs for %@", NSStringFromClass([attachment class]));
    return [SSignal single:@[]];
...
+ (SSignal *)remoteOriginInfo:(TGMediaOriginInfo *)origin
{
    switch (origin.type) {
        ...
        {
            SSignal *accessHashSignal = [SSignal defer:^SSignal *{
                TGConversation *conversation = [TGDatabaseInstance() loadConversationWithId:origin.cid.longLongValue];
                int64_t accessHash = conversation.accessHash;
                while (false) TGLog(@"IOS6MEDIA origin refresh peer=%lld mid=%d accessHash=%lld", origin.cid.longLongValue, origin.mid.intValue, accessHash);
                return [SSignal single:@(accessHash)];
            }];
            ...
            }] mapToSignal:^SSignal *(NSArray *messages)
            {
                ...
                for (TGMediaAttachment *attachment in message.mediaAttachments)
                {
                    ...
                    {
                        originInfo = ((TGAudioMediaAttachment *)attachment).ios6OriginInfo;
                        identifier = ((TGAudioMediaAttachment *)attachment).audioId;
                        break;
                    }
        ...
        {
            if (origin.stickerPackId == nil || origin.stickerPackAccessHash == nil)
                return [SSignal single:@{}];

            TGStickerPackIdReference *reference = [[TGStickerPackIdReference alloc] initWithPackId:origin.stickerPackId.longLongValue packAccessHash:origin.stickerPackAccessHash.longLongValue shortName:nil];
            return [[TGStickersSignals stickerPackInfo:reference] map:^id(TGStickerPack *pack) {
                NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
                if (![pack isKindOfClass:[TGStickerPack class]])
                    return dict;
                for (TGDocumentMediaAttachment *attachment in pack.documents)
                {
                    if (attachment.originInfo != nil)
                        dict[@(attachment.documentId)] = attachment.originInfo;
                }
                return dict;
            }];
        }
        ...
        case TGMediaOriginTypeRecentSticker:
        {
            return [[TGRecentStickersSignal remoteRecentStickers] map:^id(NSDictionary *recentStickers) {
                NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
                if (![recentStickers isKindOfClass:[NSDictionary class]])
                    return dict;
        ...
        case TGMediaOriginTypeProfilePhoto:
        {
            ...
            getPhotos.user_id = [TGTelegraphInstance createInputUserForUid:origin.profilePhotoUserId.intValue];
            getPhotos.offset = origin.profilePhotoOffset.intValue;
            getPhotos.limit = 1;
            getPhotos.max_id = 0;
            ...
            return [[[TGTelegramNetworking instance] requestSignal:getPhotos] map:^id(TLphotos_Photos *result) {
                ...
                if ([result.photos.firstObject isKindOfClass:[TLPhoto$photo class]])
                {
                    ...
                    TGMediaOriginInfo *originInfo = [TGMediaOriginInfo mediaOriginInfoWithFileReference:nil fileReferences:fileReferences userId:origin.profilePhotoUserId.intValue offset:origin.profilePhotoOffset.intValue];
                    return @{ @0: originInfo};
                }
                return nil;
            }];
        ...
        {
            int64_t peerId = origin.chatPhotoPeerId.longLongValue;
