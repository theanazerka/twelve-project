...
- (CGSize)originalSize
{
    AVAssetTrack *track = self.tracks.count == 0 ? nil : [self.tracks objectAtIndex:0];
    return track == nil ? CGSizeZero : CGRectApplyAffineTransform((CGRect){ CGPointZero, track.naturalSize }, track.preferredTransform).size;
}
