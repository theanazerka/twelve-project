...
#endif
    {
        // An app linked with the iOS 6 SDK uses the legacy text engine even
        // when running on iOS 7-10. Its contentSize already includes the
        // correct one-line padding; the TextKit fudge factor makes it look
        // like a two-line input field.
...
- (void)refreshAttributes {
#if __IPHONE_OS_VERSION_MAX_ALLOWED < 70000
    // Apps linked with the iOS 6 SDK keep UIKit's legacy UIWebDocumentView
    // text engine even on iOS 7-10. TextKit selectors such as
    // setTypingAttributes: and textStorage are not safe in that mode.
    return;
#else
    if (iosMajorVersion() < 7)
        return;
    ...
    __block NSMutableArray *inputTextTags = [[NSMutableArray alloc] init];
    ...
    if (inputTextTags != nil) {
        ...
        NSMutableSet *removeTags = [[NSMutableSet alloc] init];
...
- (NSString *)textWithEntities:(__autoreleasing NSArray ** )entities {
    NSAttributedString *string = self.attributedText;
