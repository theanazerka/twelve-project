...
+ (instancetype)mediaOriginInfoWithStringRepresentation:(NSString *)string
{
    ...
    info->_type = (TGMediaOriginType)[[keyComponents objectAtIndex:0] intValue];
    switch (info->_type)
    {
        ...
        case TGMediaOriginTypeMessage:
            // This string is passed through thumbnail URIs. On 32-bit iOS,
            // integerValue truncates modern channel peer ids, which prevents a
            // FILE_REFERENCE refresh for media received from other clients.
            info->_cid = @([keyComponents[1] longLongValue]);
            info->_mid = @([keyComponents[2] intValue]);
            break;
            
        case TGMediaOriginTypeSticker:
            info->_stickerPackId = @([keyComponents[1] longLongValue]);
            info->_stickerPackAccessHash = @([keyComponents[2] longLongValue]);
            break;
            
        case TGMediaOriginTypeProfilePhoto:
            info->_profilePhotoUserId = @([keyComponents[1] longLongValue]);
            info->_profilePhotoOffset = @([keyComponents[2] intValue]);
            break;
            
        case TGMediaOriginTypeChatPhoto:
            info->_chatPhotoPeerId = @([keyComponents[1] longLongValue]);
            break;
        ...
        case TGMediaOriginTypeWallpaper:
            info->_wallpaperId = @([keyComponents[1] longLongValue]);
            break;
    ...
    if (components.count > 2 && [components[2] length] > 0)
    {
        ...
        for (NSString *ref in refComponents)
        {
            ...
            if (components.count == 2)
                [fileReferences setObject:[NSData dataWithHexString:[components lastObject]] forKey:[components objectAtIndex:0]];
        }
    }
