...
void VoIPController::SendInit(){
	{
		...
		LOGI("IOS6VOIP init.begin seq=%u endpoints=%u layer=%d useMTProto2=%d outgoing=%d", initSeq, (unsigned int)endpoints.size(), connectionMaxLayer, (int)useMTProto2, (int)isOutgoing);
		for(shared_ptr<Endpoint>& e:endpoints){
			...
			LOGI("IOS6VOIP init.queue seq=%u endpoint=%lld type=%d addr=%s v6=%s port=%u len=%u", initSeq, (long long)e->id, (int)e->type, e->address.ToString().c_str(), e->v6address.ToString().c_str(), (unsigned int)e->port, (unsigned int)out.GetLength());
		}
	}
...
void VoIPController::ProcessIncomingPacket(NetworkPacket &packet, shared_ptr<Endpoint> srcEndpoint){
	...
	LOGI("IOS6VOIP recv.packet endpoint=%lld type=%d addr=%s v6=%s port=%u len=%u state=%d mtproto2=%d first=%02X%02X%02X%02X", (long long)srcEndpoint->id, (int)srcEndpoint->type, srcEndpoint->address.ToString().c_str(), srcEndpoint->v6address.ToString().c_str(), (unsigned int)srcEndpoint->port, (unsigned int)len, (int)state, (int)useMTProto2, buffer[0], buffer[1], buffer[2], buffer[3]);
	if(memcmp(buffer, srcEndpoint->type==Endpoint::TYPE_UDP_RELAY || srcEndpoint->type==Endpoint::TYPE_TCP_RELAY ? (void*)srcEndpoint->peerTag : (void*)callID, 16)!=0){
		LOGW("IOS6VOIP recv.drop.wrongPeerTag endpoint=%lld type=%d addr=%s port=%u len=%u state=%d first=%02X%02X%02X%02X expected=%02X%02X%02X%02X", (long long)srcEndpoint->id, (int)srcEndpoint->type, srcEndpoint->address.ToString().c_str(), (unsigned int)srcEndpoint->port, (unsigned int)len, (int)state, buffer[0], buffer[1], buffer[2], buffer[3], srcEndpoint->type==Endpoint::TYPE_UDP_RELAY || srcEndpoint->type==Endpoint::TYPE_TCP_RELAY ? srcEndpoint->peerTag[0] : callID[0], srcEndpoint->type==Endpoint::TYPE_UDP_RELAY || srcEndpoint->type==Endpoint::TYPE_TCP_RELAY ? srcEndpoint->peerTag[1] : callID[1], srcEndpoint->type==Endpoint::TYPE_UDP_RELAY || srcEndpoint->type==Endpoint::TYPE_TCP_RELAY ? srcEndpoint->peerTag[2] : callID[2], srcEndpoint->type==Endpoint::TYPE_UDP_RELAY || srcEndpoint->type==Endpoint::TYPE_TCP_RELAY ? srcEndpoint->peerTag[3] : callID[3]);
		return;
	}
	in.Seek(16);
	   ...
	   && *reinterpret_cast<uint64_t*>(buffer+16)==0xFFFFFFFFFFFFFFFFLL && *reinterpret_cast<uint32_t*>(buffer+24)==0xFFFFFFFF){
		...
		if(tlid==TLID_UDP_REFLECTOR_SELF_INFO){
			if(srcEndpoint->type==Endpoint::TYPE_UDP_RELAY /*&& udpConnectivityState==UDP_PING_SENT*/ && in.Remaining()>=32){
				...
				LOGI("IOS6VOIP recv.relaySelf endpoint=%lld addr=%s port=%u query=%lld myPort=%d", (long long)srcEndpoint->id, srcEndpoint->address.ToString().c_str(), (unsigned int)srcEndpoint->port, queryID, myPort);
	...
	if(in.Remaining()<40){
		LOGW("IOS6VOIP recv.drop.tooSmall endpoint=%lld len=%u remaining=%u state=%d", (long long)srcEndpoint->id, (unsigned int)len, (unsigned int)in.Remaining(), (int)state);
		return;
	}
	...
	if(!useMTProto2){
		...
		if(memcmp(fingerprint, keyFingerprint, 8)!=0){
			LOGW("IOS6VOIP recv.drop.wrongFingerprint.v1 endpoint=%lld state=%d got=%02X%02X%02X%02X expected=%02X%02X%02X%02X", (long long)srcEndpoint->id, (int)state, fingerprint[0], fingerprint[1], fingerprint[2], fingerprint[3], keyFingerprint[0], keyFingerprint[1], keyFingerprint[2], keyFingerprint[3]);
			return;
		}
		...
		if(memcmp(msgHash, sha+(SHA1_LENGTH-16), 16)!=0){
			LOGW("IOS6VOIP recv.drop.wrongHash.v1 endpoint=%lld state=%d retryWith2=%d remaining=%u", (long long)srcEndpoint->id, (int)state, (int)(state==STATE_WAIT_INIT || state==STATE_WAIT_INIT_ACK), (unsigned int)in.Remaining());
			if(state==STATE_WAIT_INIT || state==STATE_WAIT_INIT_ACK)
				retryWith2=true;
			else
				return;
	...
	if(useMTProto2 || retryWith2){
		...
		if(!shortFormat){
			in.ReadBytes(fingerprint, 8);
			if(memcmp(fingerprint, keyFingerprint, 8)!=0){
				LOGW("IOS6VOIP recv.drop.wrongFingerprint.v2 endpoint=%lld state=%d got=%02X%02X%02X%02X expected=%02X%02X%02X%02X", (long long)srcEndpoint->id, (int)state, fingerprint[0], fingerprint[1], fingerprint[2], fingerprint[3], keyFingerprint[0], keyFingerprint[1], keyFingerprint[2], keyFingerprint[3]);
				return;
			}
		}
		...
		if(memcmp(msgKey, msgKeyLarge+8, 16)!=0){
			LOGW("IOS6VOIP recv.drop.wrongHash.v2 endpoint=%lld state=%d decryptedLen=%u", (long long)srcEndpoint->id, (int)state, (unsigned int)decryptedLen);
			return;
		}

		uint32_t innerLen = shortFormat ? (uint32_t)in.ReadInt16() : (uint32_t)in.ReadInt32();
...
void VoIPController::SendPacket(unsigned char *data, size_t len, shared_ptr<Endpoint> ep, PendingOutgoingPacket& srcPacket){
	...
	if(srcPacket.type==PKT_INIT || state==STATE_WAIT_INIT_ACK){
		const unsigned char *expectedPrefix = (ep->type==Endpoint::TYPE_UDP_RELAY || ep->type==Endpoint::TYPE_TCP_RELAY) ? (const unsigned char *)ep->peerTag : (const unsigned char *)callID;
		bool prefixMatches = pkt.length >= 16 && memcmp(pkt.data, expectedPrefix, 16) == 0;
		LOGI("IOS6VOIP send.packet type=%u seq=%u endpoint=%lld relayType=%d addr=%s port=%u payload=%u encrypted=%u mtproto2=%d prefixMatch=%d prefix=%02X%02X%02X%02X expected=%02X%02X%02X%02X", (unsigned int)srcPacket.type, (unsigned int)srcPacket.seq, (long long)ep->id, (int)ep->type, ep->GetAddress().ToString().c_str(), (unsigned int)ep->port, (unsigned int)len, (unsigned int)pkt.length, (int)useMTProto2, prefixMatches ? 1 : 0, pkt.data[0], pkt.data[1], pkt.data[2], pkt.data[3], expectedPrefix[0], expectedPrefix[1], expectedPrefix[2], expectedPrefix[3]);
	}
...
void VoIPController::AddIPv6Relays(){
	LOGI("IOS6VOIP ipv6Relays.disabled");
	return;
