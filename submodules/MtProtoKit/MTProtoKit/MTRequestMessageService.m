...
static NSString *MTDebugHexString(NSData *data, NSUInteger limit)
{
    if (data.length == 0)
        return @"";
    ...
    if (data.length > limit)
        [string appendFormat:@"...(+%lu)", (unsigned long)(data.length - limit)];
...
{    
    ...
    if (initializeApi && _apiEnvironment != nil)
    {
        ...
        NSLog(@"IOS6AUTH initConnection serialize layer=%d apiId=%d deviceModel=%@ systemVersion=%@ appVersion=%@ systemLang=%@ langPack=%@ langPackCode=%@ flags=%d", (int)[_serialization currentLayer], (int)_apiEnvironment.apiId, _apiEnvironment.deviceModel, _apiEnvironment.systemVersion, _apiEnvironment.appVersion, _apiEnvironment.systemLangCode, _apiEnvironment.langPack, _apiEnvironment.langPackCode, flags);
            ...
            for (MTRequest *request in _requests)
            {
                ...
                if (request.requestContext != nil && request.requestContext.messageId == rpcResultMessage.requestMessageId)
                {
                    ...
                    else
                    {
                        ...
                        if (rpcResult == nil)
                        {
                            NSLog(@"IOS6AUTH parseFail messageId=%" PRId64 " bytesLength=%lu", request.requestContext.messageId, (unsigned long)unwrappedData.length);
                    ...
                    {
                        NSLog(@"IOS6AUTH rpcError messageId=%" PRId64 " code=%d description=%@", request.requestContext.messageId, (int)rpcError.errorCode, rpcError.errorDescription);
                        if (MTLogEnabled()) {
