...
- (instancetype)initWithTimestamp:(int32_t)timestamp expirationDate:(int32_t)expirationDate addressList:(NSArray *)addressList {
    self = [super init];
    if (self != nil) {
    ...
    if (result != nil) {
        ...
        if (signature == 0xd997c3c5) {
            ...
            for (int32_t i = 0; i < count; i++) {
                ...
                NSMutableArray *ruleAddressList = [[NSMutableArray alloc] init];
...
NSData * MTPBKDF2(NSData * data, NSData * salt, int rounds) {
    if (rounds < 2) {
        return nil;
    }
    const size_t hashLength = 64;
    NSMutableData *result = [[NSMutableData alloc] initWithLength:hashLength];
    int status = CCKeyDerivationPBKDF(kCCPBKDF2, data.bytes, data.length, salt.bytes, salt.length, kCCPRFHmacAlgSHA512, rounds, result.mutableBytes, hashLength);
