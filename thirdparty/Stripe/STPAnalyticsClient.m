...
// A messaging client must not emit third-party payment telemetry.
static BOOL STPAnalyticsCollectionDisabled = YES;

@interface STPAnalyticsClient()

@property (nonatomic) NSSet *apiUsage;
@property (nonatomic, readwrite) id urlSession;
...
- (void)logPayload:(NSDictionary *)payload {
    // Deliberately retained as a no-op so callers need no special handling.
    (void)payload;
}
