...
_transport(EncryptedConnection::Type::Transport, encryptionKey, sendTransportServiceAsync),
...
void NetworkManager::start() {
	_localIceParameters.ufrag = "ios6ufrag";
	_localIceParameters.pwd = "ios6-password-ios6-password";
	_localIceParameters.supportsRenomination = true;
	syslog(LOG_NOTICE, "IOS6WEBRTC cxx.network.localIce ufragLen=%lu pwdLen=%lu fromServer=%d",
	...
	if (_sendSignalingMessage) {
		...
		candidates.iceParameters = _localIceParameters;
		int component = 1;
		for (const auto &server : _rtcServers) {
			...
			char sdp[512];
			const unsigned int foundation = server.id != 0 ? server.id : (unsigned int)(component + 1);
			const unsigned int priority = 2130706431U - (unsigned int)component;
			snprintf(sdp, sizeof(sdp),
				"candidate:%u %d udp %u %s %u typ relay raddr 0.0.0.0 rport 0 generation 0 ufrag %s network-id %d",
				foundation,
				component,
				priority,
				server.host.c_str(),
			...
			RTC_LOG(LS_INFO) << "IOS6WEBRTC network.stub.syntheticCandidate sdp=" << sdp;
			syslog(LOG_NOTICE, "IOS6WEBRTC cxx.network.syntheticCandidate index=%d host=%s port=%u sdpLen=%lu",
				component,
				server.host.c_str(),
				(unsigned int)server.port,
		...
		syslog(LOG_NOTICE, "IOS6WEBRTC cxx.network.stub.start candidates=%lu servers=%lu",
	...
	if (_stateUpdated) {
		...
		RTC_LOG(LS_INFO) << "IOS6WEBRTC network.stub.ready localCandidatesSent=1 rtcServers=" << _rtcServers.size();
		_stateUpdated(state);
	}
}

void NetworkManager::receiveSignalingMessage(DecryptedMessage &&message) {
	if (const CandidatesListMessage *candidates = absl::get_if<CandidatesListMessage>(&message.message.data)) {
		_remoteIceParameters = candidates->iceParameters;
		RTC_LOG(LS_INFO) << "IOS6WEBRTC network.stub.remoteCandidates count=" << candidates->candidates.size()
	...
	RTC_LOG(LS_INFO) << "IOS6WEBRTC network.stub.unexpectedSignaling";
}

uint32_t NetworkManager::sendMessage(const Message &message) {
	if (_sendSignalingMessage) {
		if (const CandidatesListMessage *candidates = absl::get_if<CandidatesListMessage>(&message.data)) {
			Message copy;
			copy.data = *candidates;
			RTC_LOG(LS_INFO) << "IOS6WEBRTC network.stub.forwardCandidates count=" << candidates->candidates.size()
				<< " ufragLen=" << candidates->iceParameters.ufrag.size()
				<< " pwdLen=" << candidates->iceParameters.pwd.size();
			_sendSignalingMessage(std::move(copy));
		} else if (absl::get_if<AudioDataMessage>(&message.data)) {
			RTC_LOG(LS_INFO) << "IOS6WEBRTC network.stub.dropAudioTransport";
		} else {
			RTC_LOG(LS_INFO) << "IOS6WEBRTC network.stub.dropNonCandidateTransport";
		}
	}
	return 0;
}
...
void NetworkManager::candidateGathered(cricket::IceTransportInternal *, const cricket::Candidate &) {
}

void NetworkManager::candidateGatheringState(cricket::IceTransportInternal *) {
}

void NetworkManager::transportStateChanged(cricket::IceTransportInternal *) {
}

void NetworkManager::transportReadyToSend(cricket::IceTransportInternal *) {
}

void NetworkManager::transportPacketReceived(rtc::PacketTransportInternal *, const char *, size_t, const int64_t &, int) {
}

void NetworkManager::transportRouteChanged(absl::optional<rtc::NetworkRoute>) {
}
